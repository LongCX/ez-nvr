...
def start_recording(cam_config, stop_flag):
    ...
    cmd = f"ffmpeg -hide_banner -y -loglevel error -rtsp_transport tcp -use_wallclock_as_timestamps 1 -i \"{rtsp_url}\" -c:v libx264 -preset fast -crf 23 -c:a aac -b:a 128k -f segment -reset_timestamps 1 -segment_time {interval} -segment_format mp4 -segment_atclocktime 1 -strftime 1 {raw_path}/%Y-%m-%dT%H-%M-%S.mp4"
